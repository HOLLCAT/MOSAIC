...
async def test_get_study_dash(client):
    jwt_token = generate_jwt(TEST_USER["email"], TEST_USER["user_id"], TEST_USER["role"])
    expected_study = StudyResponse(**SAMPLE_STUDY)
    with patch(
        "src.dashboard.router.service.get_user_studies", new_callable=AsyncMock
    ) as mock_get_user_studies:
